...
function awesome_button( $atts, $content = null )
{
	...
	if($url) {
      return '<div class="awesome ' . $size . ' ' . $color . ' "><a href="' . $url . '">' . $text . do_shortcode($content) . '</a></div>';
	} else {
		return '<div class="awesome ' . $size . ' ' . $color . ' ">' . $text . do_shortcode($content) . '</div>';
	}
}
...
function tooltip( $atts, $content = null )
{
	...
	// convert the shortcode to this on the front end
	if($tooltext) {
		return '<a href="#" class="tooltip" title=" ' . $title . ' ">' . $tooltext . '</a>';
	} else {
		return '<a href="#" class="tooltip" title=" ' . $title . ' ">' . do_shortcode($content) . '</a>';
	}
}
